...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function createDshbCore(win) {
			...
			/** Title/description pass for one stacked text column: the first
			 * non-interactive text child becomes the item title, the second the
			 * item description. Shared by settings rows, scan-root rows and the
			 * caption columns found inside cards on opted-in pages. */
			function tagTextColumn(col) {
				tag(col, "itemText");
				let titled = false;
			...
			function scanRow(row) {
				...
				if (best !== null) tagTextColumn(best);
			...
			/**
			 * Caption-column pass: a card whose content is a stacked text column
			 * (title over explanation — e.g. archived-conversations rows, or a
			 * feature card whose body button carries a name over a description)
			 * gets the same item-text / item-title / item-desc roles as a
			 * settings row, so the caption gap and typography stay in the design
			 * language everywhere. Cards that are themselves a single control
			 * (a theme cube, a market tile, an expandable row) are left alone.
			 */
			function scanCaptionColumns(root) {
				for (const card of root.querySelectorAll("[" + attr("card") + "]")) {
					if (isInteractive(card)) continue;
					for (const child of card.children) {
						if (hasRole(child, "itemText") || hasRole(child, "row") || hasRole(child, "card")) continue;
						...
						for (const kid of child.children) {
							if (isInteractive(kid) || kid.tagName === "svg") continue;
							if (kid.textContent.trim().length > 0 && textOf(kid, 0, 3) > 0) textKids += 1;
						}
						if (textKids < 2) continue;
						tagTextColumn(child);
					}
				}
			}
			...
			/** The active section's root element identity (section-switch detector). */
			let lastSectionRoot = null;
			...
			function scanSettingsPanel() {
				...
				// The dialog keeps its scroll position across section switches;
				// reset it so a new page always opens from the top.
				const sectionRoot = sectionHost.firstElementChild;
				if (sectionRoot !== lastSectionRoot) {
					lastSectionRoot = sectionRoot;
					if (optionsEl !== null && optionsEl.scrollTop !== 0) optionsEl.scrollTop = 0;
				}
				tag(sectionHost, "section");
				scanRoot(sectionHost, true);
				// Feature cards (agent presets, desktop profile, plugin market,
				// …) keep a larger title→caption gap unless we normalize it here
				// too — the same caption-column treatment applies to every
				// settings-panel section, not just opted-in pages.
				scanCaptionColumns(sectionHost);
				return true;
			}
			/** Generic pass for opted-in third-party pages (also when the page is
			 * hosted inside the settings dialog as a section root). */
			function scanGenericRoots() {
				for (const root of doc.querySelectorAll("[" + attr("scan") + "]")) {
					// Skip nested opt-ins: the outermost root already covers them.
					if (root.parentElement !== null && root.parentElement.closest("[" + attr("scan") + "]") !== null) continue;
					scanRoot(root, false);
					scanCaptionColumns(root);
				}
			}
...
:is([data-dshb-panel], [data-dshb-scan]) {
  ...
  --dshb-gap-title-desc: 4px; /* 标题与其下方注释的统一间距(与 DSH 行内标题-注释一致) */
  --dshb-card-bg: var(--dsw-alias-bg-layer-3); /* elevated over the layer-2 panel */
...
:is([data-dshb-panel], [data-dshb-scan]) [data-dshb-header] { padding: 24px 22px 8px 20px; }
...
:is([data-dshb-panel], [data-dshb-scan]) [data-dshb-options] {
  padding-top: 22px; /* 右列顶部与左菜单顶部平行：54 + 22 = 76 = 24 + 28 + 24 */
  padding-right: 28px;
...
/* ---------- caption gap: 标题与注释永远保持同一距离 ---------- */
/* 文字列容器统一为纵向 flex + 4px 间隔(DSH 行文本列已是该形态,逐像素对齐;
   第三方/opt-in 页面的卡片内文字列(标题在上、注释在下)也被归一到同一间距)。 */
:is([data-dshb-panel], [data-dshb-scan]) [data-dshb-item-text] {
  display: flex;
  flex-direction: column;
  gap: var(--dshb-gap-title-desc);
}
:is([data-dshb-panel], [data-dshb-scan]) [data-dshb-item-text] > [data-dshb-item-title],
:is([data-dshb-panel], [data-dshb-scan]) [data-dshb-item-text] > [data-dshb-item-desc] {
  margin: 0;
}
/* 未被文字列包裹的注释(手工标记页面):用 margin 补出同样 4px */
:is([data-dshb-panel], [data-dshb-scan]) [data-dshb-item-desc]:not([data-dshb-item-text] [data-dshb-item-desc]) {
  margin-top: var(--dshb-gap-title-desc);
}
/* 页面/分组标题与其说明段落:仅在 opt-in 页面上归一(自带页面的节奏由各自
   排版决定,这里不动)——标题下紧贴的说明段与标题间统一为 4px */
[data-dshb-scan] [data-dshb-page-title]:has(+ [data-dshb-page-desc]),
[data-dshb-scan] [data-dshb-group-title]:has(+ [data-dshb-group-desc]) {
  margin-bottom: 0;
}
[data-dshb-scan] [data-dshb-page-desc],
[data-dshb-scan] [data-dshb-group-desc] {
  margin-top: var(--dshb-gap-title-desc);
}
...
/* ---------- plugin market polish (stable dshMarket* classes) ---------- */
:is([data-dshb-panel], [data-dshb-scan]) .dshMarketCategories > span:first-child {
  color: var(--dsw-alias-label-tertiary);
  font-size: 12px;
  line-height: 18px;
  font-weight: 500;
}
:is([data-dshb-panel], [data-dshb-scan]) .dshMarketCategories button {
  height: 26px;
  padding: 0 10px;
  border: 1px solid var(--dsw-alias-border-l2);
  border-radius: 999px;
  ...
  transition:
    background-color var(--dshb-duration) var(--dshb-ease),
    border-color var(--dshb-duration) var(--dshb-ease),
    color var(--dshb-duration) var(--dshb-ease);
}
:is([data-dshb-panel], [data-dshb-scan]) .dshMarketCategories button:hover {
  background: var(--dshb-hover);
}
:is([data-dshb-panel], [data-dshb-scan]) .dshMarketCategories button[aria-pressed="true"] {
  background: color-mix(in srgb, var(--dshb-focus) 12%, transparent);
  border-color: var(--dshb-focus);
  color: var(--dsw-alias-label-primary);
  font-weight: 500;
}
:is([data-dshb-panel], [data-dshb-scan]) .dshMarketTags {
  gap: 6px;
  margin-top: 12px;
}
:is([data-dshb-panel], [data-dshb-scan]) .dshMarketTags > * {
...
:is([data-dshb-panel], [data-dshb-scan]) .dshMarketTags > *:first-child {
  color: var(--dsw-alias-label-secondary);
}
:is([data-dshb-panel], [data-dshb-scan]) .dshMarketToolbar {
  margin-bottom: 18px;
}
:is([data-dshb-panel], [data-dshb-scan]) .dshMarketToolbar > span:last-child {
  height: 26px;
  padding: 0 10px;
  border: 1px solid var(--dsw-alias-border-l2);
  border-radius: 999px;
