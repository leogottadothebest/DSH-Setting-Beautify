...
:is([data-dshb-panel], [data-dshb-scan]) [data-dshb-nav] {
  width: 200px;
  gap: 40px; /* 上面（设置标题）与下面（菜单栏）之间的空间留大 */
  padding-top: 28px;
}
...
:is([data-dshb-panel], [data-dshb-scan]) [data-dshb-header] { padding: 28px 22px 8px 20px; }
...
:is([data-dshb-panel], [data-dshb-scan]) [data-dshb-options] {
  padding-top: 42px; /* 与左侧菜单栏顶部平行：头部盒(0..54) + 42 = 96 = 28 + 28 + 40 */
  padding-right: 28px;
  padding-bottom: 28px;
